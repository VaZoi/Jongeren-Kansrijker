...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db->editJongereInstituut($jongereID, $instituutID);
...
<head>
    ...
    <title>Jongere aan Instituut Koppelen</title>
...
<header>
    ...
    <div class="container">
        <h2>Jongere aan Instituut Koppelen</h2>

        <form method="post" action="KoppelInstituut.php">
            ...
            <label for="instituutID">Selecteer Instituut:</label>
            <select name="instituutID" required>
                <?php foreach ($instituten as $instituut) : ?>
                    <option value="<?php echo $instituut['instituutID']; ?>"><?php echo $instituut['instituutNaam']; ?></option>
                <?php endforeach; ?>
            </select>

            <button type="submit">Koppel Jongere aan Instituut</button>
        </form>
    </div>
